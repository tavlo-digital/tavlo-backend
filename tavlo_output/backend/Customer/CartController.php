...
class CartController extends Controller
{
    ...
    public function addItem(Request $request): JsonResponse
    {
        ...
        if ($existing) {
            ...
            // ISSUE-013: Snapshot price at add-time — prevents vendor price changes
            // from silently affecting an active cart mid-session.
            $snapshotPrice = ($menuItem->has_discount && $menuItem->discounted_price !== null)
            ...
            $item = CartItem::create([
                ...
                'stored_unit_price'     => $snapshotPrice,
                'order_id'              => null,
    ...
    public function createOrderDraft(Request $request): JsonResponse
    {
        ...
        // Derive currency from vendor settings — never hardcode EUR (ISSUE-005)
        $mySession->loadMissing('vendor.vendorSetting');
        $currency = $mySession->vendor?->vendorSetting?->currency ?? 'EUR';

        DB::transaction(function () use ($request, $mySession, $myTotal, $currency) {
            Order::create([
    ...
    public function updateOrder(Request $request, int $order_id): JsonResponse
    {
        ...
        if (! empty($data['shared_item'])) {
            $cartItem = CartItem::where('id', $data['shared_item'])
                ...
                ->whereNull('order_id')        // ISSUE-007: Only unconfirmed items can be shared
                ->lockForUpdate()              // ISSUE-011: Prevent concurrent share race condition
                ->first();

            if (! $cartItem) {
                return response()->json([
                    'message' => 'This item has already been confirmed and cannot be shared, or does not belong to this table.',
                ], 422);
            }
    ...
    public function createOrderConfirmed(Request $request): JsonResponse
    {
        ...
        // ISSUE-010: Validate all open cart items are still available before confirming.
        // Prevents kitchen receiving orders for items the vendor has since deactivated.
        ...
        $unavailableItems = $openItems->filter(
            fn (CartItem $item) => ! $item->menuItem?->is_active || ! $item->menuItem?->available
        );
    ...
    {
        // ISSUE-015: Standardised to lowercase — matches vendor API and TypeScript CartItemStatus type.
        if ($item->served_at) {
            return 'served';
        }
    ...
    {
        // ISSUE-013: Use the price locked at add-time, fall back to live price for legacy rows.
        if ($item->stored_unit_price !== null) {
            return (float) $item->stored_unit_price;
        }
